...
public:
    ...
    TransformFusion(const rclcpp::NodeOptions & options) : ParamServer("lidar_localization_transformFusion", options)
    {
        ...
        subLaserOdometry = create_subscription<nav_msgs::msg::Odometry>(
            "lidar_localization/mapping/odometry", qos,
        ...
        pubImuPath = create_publisher<nav_msgs::msg::Path>("lidar_localization/imu/path", qos);
    ...
    void imuOdometryHandler(const nav_msgs::msg::Odometry::SharedPtr odomMsg)
    {
        ...
        // Publish TF (2D only for Nav2 compatibility)
        geometry_msgs::msg::TransformStamped ts;
        tf2::convert(tCur, ts);
        ts.child_frame_id = baselinkFrame;

        // Convert to 2D: set roll=0, pitch=0, z=0
        tf2::Quaternion quat;
        tf2::fromMsg(ts.transform.rotation, quat);
        double roll, pitch, yaw;
        tf2::Matrix3x3(quat).getRPY(roll, pitch, yaw);
        quat.setRPY(0.0, 0.0, yaw);  // Keep only yaw
        ts.transform.rotation = tf2::toMsg(quat);
        ts.transform.translation.z = 0.0;  // Set z to 0
...
public:
    ...
    rclcpp::Subscription<std_msgs::msg::Bool>::SharedPtr subInitialPoseReceived;
    rclcpp::Publisher<nav_msgs::msg::Odometry>::SharedPtr pubImuOdometry;

    rclcpp::CallbackGroup::SharedPtr callbackGroupImu;
    rclcpp::CallbackGroup::SharedPtr callbackGroupOdom;
    rclcpp::CallbackGroup::SharedPtr callbackGroupInitialPose;
    ...
    IMUPreintegration(const rclcpp::NodeOptions & options) :
            ParamServer("lidar_localization_imu_preintegration", options)
    {
        callbackGroupImu = create_callback_group(
            rclcpp::CallbackGroupType::MutuallyExclusive);
        callbackGroupOdom = create_callback_group(
            rclcpp::CallbackGroupType::MutuallyExclusive);
        callbackGroupInitialPose = create_callback_group(
            rclcpp::CallbackGroupType::MutuallyExclusive);
        ...
        auto initialPoseOpt = rclcpp::SubscriptionOptions();
        initialPoseOpt.callback_group = callbackGroupInitialPose;
        ...
        subOdometry = create_subscription<nav_msgs::msg::Odometry>(
            ...
            odomOpt);
        subInitialPoseReceived = create_subscription<std_msgs::msg::Bool>(
            "lidar_localization/initial_pose_received", qos,
            std::bind(&IMUPreintegration::initialPoseReceivedHandler, this, std::placeholders::_1),
            initialPoseOpt);
    ...
    void initialPoseReceivedHandler(const std_msgs::msg::Bool::SharedPtr msg)
    {
        std::lock_guard<std::mutex> lock(mtx);
        if (msg->data == true)
        {
            ...
            prevPose_ = gtsam::Pose3(gtsam::Rot3(1, 0, 0, 0), gtsam::Point3(0, 0, 0));
