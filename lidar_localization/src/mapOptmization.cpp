...
#include "lidar_localization/srv/save_map.hpp"
...
public:
    ...
    rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr pubGpsIntegrationPosition;

    rclcpp::Service<lidar_localization::srv::SaveMap>::SharedPtr srvSaveMap;
    ...
    pcl::CustomVoxelGrid<PointType> downSizeFilterICP;
    ...
    mapOptimization(const rclcpp::NodeOptions & options) : ParamServer("lidar_localization_mapOptimization", options)
    {
        ...
        pubLaserCloudSurround = create_publisher<sensor_msgs::msg::PointCloud2>("lidar_localization/mapping/map_global", 1);
        ...
        auto saveMapService = [this](const std::shared_ptr<rmw_request_id_t> request_header, const std::shared_ptr<lidar_localization::srv::SaveMap::Request> req, std::shared_ptr<lidar_localization::srv::SaveMap::Response> res) -> void {
            (void)request_header;
        ...
        srvSaveMap = create_service<lidar_localization::srv::SaveMap>("lidar_localization/save_map", saveMapService);
        pubHistoryKeyFrames = create_publisher<sensor_msgs::msg::PointCloud2>("lidar_localization/mapping/icp_loop_closure_history_cloud", 1);
        pubIcpKeyFrames = create_publisher<sensor_msgs::msg::PointCloud2>("lidar_localization/mapping/icp_loop_closure_history_cloud", 1);
        pubLoopConstraintEdge = create_publisher<visualization_msgs::msg::MarkerArray>("/lidar_localization/mapping/loop_closure_constraints", 1);
        ...
        pubGpsIntegrationPosition = create_publisher<sensor_msgs::msg::PointCloud2>("/gps_integration_position", 5);
    ...
    void publishGlobalMap()
    {
        ...
        pcl::CustomVoxelGrid<PointType> downSizeFilterGlobalMapKeyPoses; // for global map visualization
        ...
        pcl::CustomVoxelGrid<PointType> downSizeFilterGlobalMapKeyFrames; // for global map visualization
    ...
    void updateInitialGuess()
    {
        ...
        if (cloudKeyPoses3D->points.empty())
        {
            ...
            if (!useImuHeadingInitialization)
                transformTobeMapped[2] = initial_heading_yaw;
    ...
    void addGPSFactor()
    {
        ...
        RCLCPP_INFO_STREAM(get_logger(), "no GPS integration period : " << no_gps_integration_period);
        ...
        {
            if (stamp2Sec(gpsQueue.front().header.stamp) < timeLaserInfoCur - 0.1)
            {
                // message too old
                gpsQueue.pop_front();
            }
            else if (stamp2Sec(gpsQueue.front().header.stamp) > timeLaserInfoCur + 0.1)
            {
                // message too new
                break;
            }
            else
            {
                ...
                {
                    if (no_gps_integration_period < 100)
                        continue;
                    else
                    {
                        noise_x = 100000.0;
                ...
                Vector3 << max(noise_x, 0.1f), max(noise_y, 0.1f), max(noise_z, 0.001f);
                ...
                publishCloud(pubGpsIntegrationPosition, gps_pointcloud, timeLaserInfoStamp, "map");

                last_gps_integration_time = stamp2Sec(thisGPS.header.stamp);

                aLoopIsClosed = true;
                gps_integrated_count++;
                RCLCPP_INFO_STREAM(get_logger(), "gps update!");
    ...
    void publishFrames()
    {
        ...
        if (pubPath->get_subscription_count() != 0 && path_count % 25 == 0)
        {
