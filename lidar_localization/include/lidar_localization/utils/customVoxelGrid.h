#ifndef CUSTOM_VOXEL_GRID_H
#define CUSTOM_VOXEL_GRID_H
...
namespace pcl
{
    ...
    class CustomVoxelGrid : public VoxelGrid<PointT>
    {
    protected:
        ...
        using PointCloud = typename VoxelGrid<PointT>::PointCloud;
        using PointCloudPtr = typename PointCloud::Ptr;
        using PointCloudConstPtr = typename PointCloud::ConstPtr;

    public:
        using Ptr = shared_ptr<CustomVoxelGrid<PointT>>;
        using ConstPtr = shared_ptr<const CustomVoxelGrid<PointT>>;

        CustomVoxelGrid()
        {
            filter_name_ = "CostomVoxelGrid";
        }

        ~CustomVoxelGrid() override = default;
        ...
        void applyFilter(PointCloud &output) override
        {
            ...
            // Modified overflow check
            if (isMultiplicationOverflow(dx, dy) || isMultiplicationOverflow(dx * dy, dz))
            {
                PCL_WARN("[pcl::%s::applyFilter] Leaf size is too small for the input dataset. Integer indices would overflow.\n", getClassName().c_str());
                output = *input_;
                return;
            ...
            if (save_leaf_layout_)
            {
                try
                {
                    ...
                    std::uint64_t new_layout_size = div_b_[0] * div_b_[1] * div_b_[2];
                    // This is the number of elements that need to be re-initialized to -1
                    std::uint64_t reinit_size = std::min(static_cast<unsigned int>(new_layout_size), static_cast<unsigned int>(leaf_layout_.size()));
                    for (std::uint64_t i = 0; i < reinit_size; i++)
                    {
                        leaf_layout_[i] = -1;
                    }
        ...
        bool isMultiplicationOverflow(std::int64_t a, std::int64_t b)
        {
            if (a == 0 || b == 0)
                return false;
